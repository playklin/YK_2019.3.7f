...
public class SpisokADMMs : MonoBehaviour {
    ...
    void Start()
    {
        StartCoroutine(GetPlataIDs(PlayerPrefs.GetString("facenumber"), results => OnReceivedModels(results))); 
    }
   ...
   #region 
    ...
    public IEnumerator GetPlataIDs(string face,System.Action<TestItemModel[]> callback){
        ...
        using (UnityWebRequest www = UnityWebRequest.Post("https://playklin.000webhostapp.com/yk/GetALLykAdmMobFace.php",form)){
    ...
    void InitializeItemView (GameObject viewGameObject, TestItemModel model)
    {
        ...
        view.clickButton.onClick.AddListener(
            ()=> 
            {
                ...
                PlayerPrefs.SetString("id_adm", view.id.text);
