...
public class McounterGBC : MonoBehaviour
{
    public InputField if_countGBC;
    public Text t_countlastGBC, t_date, t_facenumber;
    // Start is called before the first frame update
    void Start()
    {
        t_facenumber.text = "л/сч " + PlayerPrefs.GetString("facenumber");
        StartCoroutine(GetLastGBC(PlayerPrefs.GetString("facenumber")));
        StartCoroutine(GetServerDate());
    }

    public void ClickMcounter(){SceneManager.LoadScene("Mcounter");}
    public void ClickSendcountGBC(){StartCoroutine(EditLastGBC(PlayerPrefs.GetString("facenumber"),if_countGBC.text));}

    IEnumerator GetLastGBC(string facenumber){
        WWWForm form = new WWWForm(); form.AddField("_facenumber", facenumber); // correct
        using (UnityWebRequest www = UnityWebRequest.Post("https://playklin.000webhostapp.com/yk/GetLastGBC.php",form))
        {yield return www.SendWebRequest(); if (www.isNetworkError || www.isHttpError) { Debug.Log(www.error); }
        else{t_countlastGBC.text = www.downloadHandler.text;
        //PlayerPrefs.SetString("title_order", www.downloadHandler.text);
        //Debug.Log("" + www.downloadHandler.text);
        }}//StartCoroutine(GetTitle(If_id_order.text));
    }

    IEnumerator EditLastGBC(string facenumber, string countXBC){
        WWWForm form = new WWWForm(); form.AddField("_facenumber", facenumber);form.AddField("_countXBC", countXBC);
        using (UnityWebRequest www = UnityWebRequest.Post("https://playklin.000webhostapp.com/yk/EditLastGBC.php",form))
        {yield return www.SendWebRequest(); if (www.isNetworkError || www.isHttpError) { Debug.Log(www.error); }
        else{//t_countlastXBC.text = www.downloadHandler.text;
        yield return new WaitForSeconds(0.5f);
        //SceneManager.LoadScene("McounterGBC");
        //PlayerPrefs.SetString("title_order", www.downloadHandler.text);
    ...
    public void sendEMAILcountGBC()
    {
        MailMessage message = new MailMessage();

        message.Subject = "ЖИЛИЩНИК ЛЕФОРТОВО " + "(ГВС)"+ " ул." + PlayerPrefs.GetString("street") + ", " + "д." + PlayerPrefs.GetString("house") + 
                          " кв." + PlayerPrefs.GetString("flat") + ", л/с " + PlayerPrefs.GetString("facenumber");  // Тема письма
        message.Body = "" + "Счётчик № " + PlayerPrefs.GetString("GBC") + "\n" + "\n" + if_countGBC.text + "\n" + "\n" + "Контакты отправителя:\n" + PlayerPrefs.GetString("surname") + " " + PlayerPrefs.GetString("name") + 
