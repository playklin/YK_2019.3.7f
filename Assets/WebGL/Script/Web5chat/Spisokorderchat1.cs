...
public class Spisokorderchat1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //string userId = "0"; 
        //StartCoroutine (GetPlataIDs(PlayerPrefs.GetString("id_order") ,_createItemsCallback));
        StartCoroutine (GetPlataIDs(Web5.exampel ,_createItemsCallback));
    ...
    IEnumerator CreateItemsRoutine(string jsonArrayString){
        ...
        for(int i = 0; i < jsonArray.Count ; i++){
            ...
            GameObject item = Instantiate (Resources.Load("Prefabs/itemOchat") as GameObject);
            item.transform.SetParent(this.transform);
            item.transform.localScale = Vector3.one;
            item.transform.localPosition = Vector3.zero;
            // заполняем информацией
            //item.transform.Find("Id").GetComponent<Text>().text = "№ " + itemInfoJason["id"];
            item.transform.Find("Datetime").GetComponent<Text>().text = itemInfoJason["datetime"];
    ...
    public IEnumerator GetPlataIDs(string userID,System.Action<string> callbackU){//I_krug.SetActive(true);
        ...
        using (UnityWebRequest www = UnityWebRequest.Post("https://playklin.000webhostapp.com/yk/GetIDsALLykorderchat.php",form)){
    ...
    public IEnumerator GetPlata(string itemID, System.Action<string> callbackU){
        ...
        using (UnityWebRequest www = UnityWebRequest.Post("https://playklin.000webhostapp.com/yk/GetInfoIDykorderchat.php", form)){
        yield return www.SendWebRequest();
