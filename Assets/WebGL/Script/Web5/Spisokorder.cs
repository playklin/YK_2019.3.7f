...
public class Spisokorder : MonoBehaviour
{
    ...
    IEnumerator CreateItemsRoutine(string jsonArrayString){
        ...
        for(int i = 0; i < jsonArray.Count ; i++){
            ...
            GameObject item = Instantiate (Resources.Load("Prefabs/itemO") as GameObject);
            item.transform.SetParent(this.transform);
            item.transform.localScale = Vector3.one;
            item.transform.localPosition = Vector3.zero;
            // заполняем информацией
            item.transform.Find("Id").GetComponent<Text>().text = "№ " + itemInfoJason["id"];
            item.transform.Find("Status").GetComponent<Text>().text = itemInfoJason["status"];
            item.transform.Find("Date").GetComponent<Text>().text = itemInfoJason["date"];
            item.transform.Find("Title").GetComponent<Text>().text = "Тема: " + itemInfoJason["title"];
    ...
    public IEnumerator GetPlataIDs(string userID,System.Action<string> callbackU){//I_krug.SetActive(true);
        ...
        using (UnityWebRequest www = UnityWebRequest.Post("https://playklin.000webhostapp.com/yk/GetIDsALLykorder.php",form)){
    ...
    public IEnumerator GetPlata(string itemID, System.Action<string> callbackU){
        ...
        using (UnityWebRequest www = UnityWebRequest.Post("https://playklin.000webhostapp.com/yk/GetInfoIDykorder.php", form)){
        yield return www.SendWebRequest();
