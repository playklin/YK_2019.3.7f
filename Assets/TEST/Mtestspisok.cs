...
public class Mtestspisok : MonoBehaviour
{
    ...
    IEnumerator CreateItemsRoutine(string jsonArrayString){
        ...
        for(int i = 0; i < jsonArray.Count ; i++){
            ...
            GameObject item = Instantiate (Resources.Load("Prefabs/ItemPrefarb") as GameObject);
            item.transform.SetParent(this.transform);
            item.transform.localScale = Vector3.one;
            item.transform.localPosition = Vector3.zero;
            // заполняем информацией
            //item.transform.Find("Id").GetComponent<Text>().text = itemInfoJason["id"];
            //item.transform.Find("Text").GetComponent<Text>().text = itemInfoJason["date"];
            item.transform.Find("TitleText").GetComponent<Text>().text = itemInfoJason["news"];
            //item.transform.Find("ClickButton").GetComponent<Button>().text = itemInfoJason["news"];
    ...
    public IEnumerator GetPlataIDs(string userID,System.Action<string> callbackU){//I_krug.SetActive(true);
        ...
        using (UnityWebRequest www = UnityWebRequest.Post("https://playklin.000webhostapp.com/yk/GetIDsALLyknews.php",form)){
        yield return www.SendWebRequest();

            if (www.isNetworkError || www.isHttpError) { Debug.Log(www.error); }else{
            Debug.Log("получили " + www.downloadHandler.text);
    ...
    public IEnumerator GetPlata(string itemID, System.Action<string> callbackU){
        ...
        using (UnityWebRequest www = UnityWebRequest.Post("https://playklin.000webhostapp.com/yk/GetInfoIDyknews.php", form)){
        yield return www.SendWebRequest();
